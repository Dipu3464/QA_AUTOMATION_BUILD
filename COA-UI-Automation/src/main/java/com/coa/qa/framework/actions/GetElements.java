...
public class GetElements {
    ...
    public List<WebElement> getElementsFromList(List<WebElement> eles, String selectorType, String selector) {
        List<WebElement> els = new ArrayList<>();

        for (WebElement we : eles) {

            we.findElement(selectTypeObj.getelementbytype(selectorType, selector));
            els.add(we);
        }
        return els;
    }
     ...
     *
     * @param eles    : the list from which the expected element is to be extracted
     * @param eleText : Contain text of the expected element
     */
    public WebElement getElementFromListByContainText(List<WebElement> eles, String eleText) {

        for (WebElement we : eles) {

            if (we.getText().contains(eleText))
                return we;
